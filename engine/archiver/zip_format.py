...
def zip(source, dest):
    ...
    if not os.path.exists(dest_directory):
        logging.info("Created zip destination folder. %s" % dest_directory)
    ...
    if os.path.exists(source):
        ...
        if os.path.isfile(source):
            ...
            zOut.close()
            logging.info ("   Compressed: %s into %s" % (source, dest))
        ...
        logging.info("File: " + source + " Doesn't exist! zip function aborted.")
...
def unzip_single_file(source, dest):
    if os.path.isfile(source):
        ...
        logging.info("Unzip Single File function, file not found: " + source)
...
def unzip(source, dest):
    ...
    if os.path.isfile(source):
        ...
        logging.info ("Given path is a file")

        if file_extension.lower() == ZIP_EXT:
            unzip_single_file(source, dest)
    elif os.path.isdir(source):
        logging.info ("Given path is a directory")
        ...
        logging.info ("Not found: " + str(source))
...
def printDebugInfo(callerKey):
    # Debug info:
    logging.debug ("------------------------------------------------------------------")
    logging.debug ("Archiver DEBUG info, %s" % callerKey)
    logging.debug ("------------------------------------------------------------------")


# TODO Used for testing.
def main():
    logging.info("Starting Test for Zip Format")
