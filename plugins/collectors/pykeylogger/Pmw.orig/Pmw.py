...
def setversion(version):
    if version != _VERSION:
        print('Dynamic versioning not available')
        raise ValueError
...
def forwardmethods(fromClass, toClass, toPart, exclude = ()):
    ...
    if type(toPart) != types.StringType:
        ...
        # It's not a valid type
            ...
            print('toPart must be attribute name, function or method')
            raise TypeError
...
class MegaArchetype:
    ...
    def createcomponent(self, componentName, componentAliases,
        ...
        if self.__componentInfo.has_key(componentName):
            print('Component "%s" already exists' % componentName)
            raise ValueError

        if '_' in componentName:
            print('Component name "%s" must not contain "_"' % componentName)
            raise ValueError                    
    ...
    def initialiseoptions(self, dummy = None):
        ...
        if self._initialiseoptions_counter == 0:
            ...
            if len(unusedOptions) > 0:
                ...
                print (text + string.join(unusedOptions, ', ') + \
...
class MegaToplevel(MegaArchetype):
    ...
    def activate(self, globalMode = 0, geometry = 'centerscreenfirst'):
        if self._active:
            print('Window is already active')
            raise ValueError
        if self.state() == 'normal':
            self.withdraw()
...
def setbusycursorattributes(window, **kw):
    ...
    for name, value in kw.items():
        if name == 'exclude':
            ...
            print('Unknown busycursor attribute "' + name + '"')
            raise KeyError
...
def busycallback(command, updateFunction = None):
    if not callable(command):
        print('cannot register non-command busy callback %s %s' % \
...
class Dialog(MegaToplevel):
    def __init__(self, parent = None, **kw):
        ...
        if pos not in 'nsew':
            print('bad buttonboxpos option "%s":  should be n, s, e, or w' \
                    % pos)
            raise ValueError                
    ...
    def _buttons(self):
        ...
        if type(buttons) != types.TupleType and type(buttons) != types.ListType:
            print('bad buttons option "%s": should be a tuple' % str(buttons))
            raise ValueError
...
class Balloon(MegaToplevel):
    ...
    def _state(self):
        if self['state'] not in ('both', 'balloon', 'status', 'none'):
            print('bad state option ' + repr(self['state']) + \
    ...
    def _relmouse(self):
        if self['relmouse'] not in ('both', 'x', 'y', 'none'):
            print('bad relmouse option ' + repr(self['relmouse'])+ \
...
class ButtonBox(MegaWidget):
    def __init__(self, parent = None, **kw):
        ...
        if orient == 'horizontal':
            ...
            print('bad orient option ' + repr(orient) + \
    ...
    def index(self, index, forInsert = 0):
        ...
        if type(index) == types.IntType:
            if forInsert and index <= listLength:
                ...
                print('index "%s" is out of range' % index)
                raise ValueError
        elif index is END:
            if forInsert:
                return listLength
            ...
            print('bad index "%s": must be %s' % (index, validValues))
            raise ValueError
                

    def insert(self, componentName, beforeComponent = 0, **kw):
        if componentName in self.components():
            print('button "%s" already exists' % componentName)
            raise ValueError
...
class EntryField(MegaWidget):
    ...
    def _checkValidateFunction(self, function, option, validator):
        ...
        if function is not None and not callable(function):
            ...
            print(msg % (option, validator, standard, extra))
            raise ValueError
...
class MainMenuBar(MegaArchetype):
    ...
    def _addmenu(self, parentMenuName, menuName, balloonHelp, statusHelp,
        ...
        if (menuName) in self.components():
            print('menu "%s" already exists' % menuName)
            raise ValueError
    ...
    def addmenuitem(self, menuName, itemType, statusHelp = '',
        ...
        if itemType == 'command':
            ...
            print('unknown menuitem type "%s"' % itemType)
            raise ValueError
    ...
    def __init__(self, parent = None, **kw):
        ...
        if iconpos is None:
            ...
            if iconpos not in 'nsew':
                print('bad iconpos option "%s":  should be n, s, e, or w' \
                        % iconpos)
                raise ValueError
...
class NoteBook(MegaArchetype):
    ...
    def __init__(self, parent = None, **kw):
        ...
        if tabpos is not None and tabpos != 'n':
            print('bad tabpos option %s:  should be n or None' % repr(tabpos))
            raise ValueError
...
class PanedWidget(MegaWidget):
    ...
    def __init__(self, parent = None, **kw):
        ...
        if self['orient'] not in ('horizontal', 'vertical'):
            print('bad orient option ' + repr(self['orient']) + \
                ': must be either \'horizontal\' or \'vertical\'')
            raise ValueError
    ...
    def _parsePaneOptions(self, name, args):
        ...
        for arg, value in args.items():
            ...
            if arg == 'size':
                ...
                print('keyword must be "size", "min", or "max"')
                raise ValueError
    ...
    def index(self, index):
        ...
        if type(index) == types.IntType:
            ...
            if listLength > 0:
                ...
                print('RadioSelect has no buttons')
                raise ValueError
        else:
    ...
    def setvalue(self, textOrList):
        ...
        if type(textOrList) == types.StringType:
            if textOrList in listitems:
                ...
                print('no such item "%s"' % textOrList)
                raise ValueError
        else:
...
class Counter(MegaWidget):
    ...
    def _datatype(self):
        ...
        if _counterCommands.has_key(datatype):
            ...
            validValues.sort()
            print(('bad datatype value "%s":  must be a' +
...
def logicalfont(name='Helvetica', sizeIncr = 0, **kw):
  if not _fontInfo.has_key(name):
    print('font %s does not exist' % name)
    raise ValueError
